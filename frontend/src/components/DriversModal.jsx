...
export default function DriversModal({ open, onClose }) {
  ...
  async function handleSaved(name) {
    closeDrawer();
    await load();
    if (name) {
      setSavedName(name);
  ...
  return (
    ...
    >
      <div style={{
        ...
        {savedName && (
          <div style={{
            padding: '0.5rem 1.25rem', flexShrink: 0,
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0.5rem 0.5rem' }}>
          {loadError ? (
            <div style={{ color: '#DC2626', fontFamily: 'DM Mono', fontSize: '0.72rem', padding: '1.5rem 1rem', lineHeight: 1.6 }}>
