...
export default function AnalyticsPage() {
  ...
  // Savings calculations — based on dispatched runs only (optimised routes)
  // Assumptions: 20% distance saving vs unoptimised, £0.35/mile van running cost, £13/hr driver wage
  const FUEL_RATE          = 0.35;       // £ per mile (diesel + vehicle wear)
  const DRIVER_RATE        = 13 / 60;    // £ per minute
  const OPTIMISE_SAVING    = 0.20;       // 20% distance improvement over unoptimised
  const DISPATCH_MINS      = 30;         // estimated mins saved per run vs manual planning

  const dispatchedRuns     = runs.filter(r => r.status === 'dispatched');
  const optimisedMiles     = dispatchedRuns.reduce((s, r) => s + (r.total_miles || 0), 0);
  const optimisedMins      = dispatchedRuns.reduce((s, r) => s + (r.est_drive_minutes || 0), 0);

  const milesSaved         = Math.round(optimisedMiles * OPTIMISE_SAVING);
  const fuelSaved          = Math.round(milesSaved * FUEL_RATE);
  const driverMinsSaved    = Math.round(optimisedMins * OPTIMISE_SAVING);
  const driverHrsSaved     = (driverMinsSaved / 60).toFixed(1);
  const driverCostSaved    = Math.round(driverMinsSaved * DRIVER_RATE);
  const dispatchMinsSaved  = dispatchedRuns.length * DISPATCH_MINS;
              ...
              <div style={{ marginBottom: '2rem' }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.75rem', marginBottom: '1rem', flexWrap: 'wrap' }}>
                  <div style={{ fontFamily: 'DM Mono', fontSize: '0.6rem', letterSpacing: '2px', color: 'var(--light-mid)' }}>
                    ESTIMATED SAVINGS — LAST {period} DAYS
                  </div>
                  <div style={{ fontFamily: 'DM Mono', fontSize: '0.55rem', color: 'var(--light-mid)', opacity: 0.6 }}>
                    vs unoptimised manual routing · 20% efficiency gain · £0.35/mi · £13/hr driver
                  </div>
                </div>
                ...
                <div style={{
                  ...
                  <div>
                    <div style={{ fontFamily: 'DM Mono', fontSize: '0.57rem', letterSpacing: '2px', color: 'var(--green)', marginBottom: '4px' }}>
                      TOTAL ESTIMATED SAVING
                    </div>
                    <div style={{ fontFamily: 'DM Sans', fontWeight: 800, fontSize: '2.4rem', color: 'var(--green)', lineHeight: 1 }}>
                      £{totalSaved.toLocaleString()}
                    </div>
                  </div>
                  <div style={{ fontFamily: 'DM Mono', fontSize: '0.62rem', color: 'var(--light-mid)', lineHeight: 1.8, textAlign: isMobile ? 'left' : 'right' }}>
                    <div>{milesSaved} miles not driven</div>
                    <div>{driverHrsSaved} hrs driver time saved</div>
                    <div>{dispatchHrsSaved} hrs dispatch time saved</div>
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, 1fr)', gap: '1rem' }}>
                  <div style={{
                    ...
                    borderRadius: '12px', padding: '1.25rem 1.5rem', borderLeft: '4px solid var(--green)',
                  }}>
                    <div style={{ fontFamily: 'DM Mono', fontSize: '0.57rem', letterSpacing: '2px', color: 'var(--light-mid)', marginBottom: '0.6rem' }}>
                      FUEL SAVED
                    </div>
                    <div style={{ fontFamily: 'DM Sans', fontWeight: 800, fontSize: '1.8rem', color: 'var(--green)' }}>
                      £{fuelSaved.toLocaleString()}
                    </div>
                    <div style={{ fontFamily: 'DM Mono', fontSize: '0.6rem', color: 'var(--light-mid)', marginTop: '6px', lineHeight: 1.6 }}>
                      {milesSaved} miles saved across {dispatchedRuns.length} runs<br />
                      at £0.35/mile running cost
                    </div>
                  </div>

                  <div style={{
                    background: 'var(--charcoal)', border: '1px solid var(--mid)',
                    borderRadius: '12px', padding: '1.25rem 1.5rem', borderLeft: '4px solid var(--amber)',
                  }}>
                    <div style={{ fontFamily: 'DM Mono', fontSize: '0.57rem', letterSpacing: '2px', color: 'var(--light-mid)', marginBottom: '0.6rem' }}>
                      DRIVER TIME SAVED
                    </div>
                    <div style={{ fontFamily: 'DM Sans', fontWeight: 800, fontSize: '1.8rem', color: 'var(--amber)' }}>
                      {driverHrsSaved} hrs
                    </div>
                    <div style={{ fontFamily: 'DM Mono', fontSize: '0.6rem', color: 'var(--light-mid)', marginTop: '6px', lineHeight: 1.6 }}>
                      £{driverCostSaved.toLocaleString()} at £13/hr<br />
                      vs taking longest unoptimised route
                    </div>
                  </div>

                  <div style={{
                    background: 'var(--charcoal)', border: '1px solid var(--mid)',
                    borderRadius: '12px', padding: '1.25rem 1.5rem', borderLeft: '4px solid var(--rust)',
                  }}>
                    <div style={{ fontFamily: 'DM Mono', fontSize: '0.57rem', letterSpacing: '2px', color: 'var(--light-mid)', marginBottom: '0.6rem' }}>
                      DISPATCH TIME SAVED <span style={{ opacity: 0.6 }}>EST.</span>
                    </div>
                    <div style={{ fontFamily: 'DM Sans', fontWeight: 800, fontSize: '1.8rem', color: 'var(--rust)' }}>
                      {dispatchHrsSaved} hrs
                    </div>
                    <div style={{ fontFamily: 'DM Mono', fontSize: '0.6rem', color: 'var(--light-mid)', marginTop: '6px', lineHeight: 1.6 }}>
                      £{dispatchCostSaved.toLocaleString()} at £13/hr<br />
                      ~30 min saved per run vs manual planning
                    </div>
                  </div>
                </div>
              </div>
