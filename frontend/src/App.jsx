...
export default function App() {
  ...
  const handleParsed = useCallback(async (result) => {
    ...
    // reset:true means the backend replaced all stops (e.g. /example) — clear and replace
    if (result.reset) {
  ...
  const handleOptimise = useCallback(async () => {
    ...
    try {
      ...
      if (currentRunId) {
        ...
        setStops(updated);
        // Only update route_url if we got one — avoid overwriting a valid URL with null
        if (result.maps_url) {
      ...
      if (!result.maps_url) {
        setError('Route optimised but no map link generated — check that your customers have addresses saved.');
      }
    } catch (e) {
      setError(e.message);
    } finally {
